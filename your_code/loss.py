...
class SquaredLoss(Loss):
    ...
    def forward(self, X, w, y):
        ...
        if self.regularization == None:
            ...
            reg_term = self.regularization.forward(w)
    
        loss_sum = 0
        for i in range(0, len(X)):
            loss_sum += 0.5 * (y[i]-np.matmul(np.transpose(w),X[i]))**2
        loss_sum = loss_sum/len(X)
        loss_sum += reg_term
        return loss_sum
    ...
    def backward(self, X, w, y):
        ...
        if self.regularization == None:
            ...
            reg_term = self.regularization.backward(w)

        grad = np.zeros([len(w)])
        for i in range(0, len(X)):
            number = y[i]-np.matmul(np.transpose(w),X[i])
            grad = grad + number*X[i]
        grad= -1/len(X) * grad
        grad = grad + reg_term
        return grad
...
class HingeLoss(Loss):
    ...
    def forward(self, X, w, y):
        ...
        loss = 0
        for i in range(0,len(X)):
            loss += max(0,1-y[i]*np.matmul(np.transpose(w),X[i]))
        loss = loss/len(X)
        loss = loss + reg_term
        return loss
    ...
    def backward(self, X, w, y):
        ...
        grad = np.zeros([len(w)])
        for i in range(0, len(X)):
            number = 1-y[i]*np.matmul(np.transpose(w),X[i])
            if number>0:
                coeff = 1
            else:
                coeff = 0
